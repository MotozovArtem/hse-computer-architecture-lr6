function del() {
    ...
    } else if (document.getElementById('store').dataset.strategy == "LFU" || document.getElementById('store').dataset.strategy == "SC") {
        $($("#table2 tr .tag2")[i]).html(tag1)
        $($("#table3 tr .data")[i]).html(string)
        $($("#table4 tr .string")[16]).html(string)
    ...
    if (document.getElementById('store').dataset.strategy == "SC") {
        sortTableSecondChance('table4', 2);
        ('table4', 1);
    }
    ...
    if (tag1 != "D68") {
        //переписать в будущем для большого числа вариантов и с любой последней строкой
